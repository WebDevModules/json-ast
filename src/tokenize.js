...
function isExp(char) { return char === 'e' || char === 'E'; }
...
function parseKeyword(source, index, line, column) {
  const matched = Object.keys(keywordsTokens)
                      .find(name => name === source.substr(index, name.length));
...
export function tokenize(source, settings) {
  ...
  while (index < source.length) {
    ...
    if (matched) {
      ...
      error(tokenizeErrorTypes.cannotTokenizeSymbol(source.charAt(index), line,
                                                    column),
            source, line, column);
    }
  }
